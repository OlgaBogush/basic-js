  ...
  cryptography(message, key, isDecrypt = false) {
    ...
    for (let i = 0; i < messageLength; i++) {
      if (message.codePointAt(i) > 64 && message.codePointAt(i) < 91) {
        if (keyLetterNumber === keyLength) keyLetterNumber = 0
        result += isDecrypt
          ? String.fromCodePoint(
              ((message.codePointAt(i) +
                26 -
                key.codePointAt(keyLetterNumber)) %
                26) +
                65
            )
          : String.fromCodePoint(
              ((message.codePointAt(i) + key.codePointAt(keyLetterNumber)) %
                26) +
                65
            )
        keyLetterNumber++
